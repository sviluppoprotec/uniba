...
namespace TheSite.AnagrafeImpianti
{
    ...
    public class NavigazioneServizi : System.Web.UI.Page
    {
        ...
        protected Repeater RepeaterEdificio;
        ...
        {
            RepeaterEdificio.DataSource = Ds.Tables[0];
            RepeaterEdificio.DataBind();
        }


        protected void RepeaterEdificio_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                var curRow = ((DataTable)((Repeater)sender).DataSource).Rows[e.Item.ItemIndex];
                DataTable DrCollection = DatiEdificioT(Int32.Parse(curRow["ID"].ToString()));
                var repeaterServizi = (Repeater)e.Item.FindControl("RepeaterServizi");
                string servizio = string.Empty;
                DataTable dt = new DataTable();
                dt.Columns.Add(new DataColumn("ID"));
                dt.Columns.Add(new DataColumn("DESCRIZIONE"));
                dt.Columns.Add(new DataColumn("BL_ID"));
                foreach (DataRow DrDettagli in DrCollection.Rows)
                {
                    if (DrDettagli["servizi_id"].ToString() == cmbsServizio.SelectedValue || cmbsServizio.SelectedIndex == 0)
                    {
                        ...
                        {
                            var dr = dt.NewRow();
                            dr["ID"] = DrDettagli["servizi_id"];
                            dr["DESCRIZIONE"] = DrDettagli["descrizione"];
                            dr["BL_ID"] = curRow["ID"];
                            dt.Rows.Add(dr);
                ...
                repeaterServizi.DataSource = dt;
                repeaterServizi.DataBind();
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        private DataTable DatiEdificioT(Int32 bl_id)
        {
            S_ControlsCollection CollezioneControlli = new S_ControlsCollection();
